...
{
	//printf("ahhhhhhhhhhhhhhhhhhhhhh%d\n",tree->coord_passed[node->coord_index]);
        ...
        if (node->coord != NULL) {
                ...
                for (i = 0; i < dim; i++) {
                        if (i != dim - 1) {
                                ...
                                ///printf("%.2f),%ld\n", node->coord[i],node->coord_index);//found it!!!!!!!!!!!!!!!!
                        }
                }
        } else {
                ///printf("(none)\n");
...
/*
void kdtree_insert(struct kdtree *tree, double *coord, int i)
{
        if (tree->count + 1 > tree->capacity) {
                resize(tree);
        }
...
{
	//printf("ttttttttttttttttttttarget=%f\n",target[0]);
        double dist = distance(node->coord, target, tree->dim);
		//printf("%f,dist=========%f,knn_num=%d\n",node->coord[0],dist,tree->knn_num);
        if (tree->knn_num < k) {
			//printf("knn_num=%d,k=%d\n",tree->knn_num,k);
...
static void kdtree_search_recursive(struct kdtree *tree, struct kdnode *node, double *target, int k, int *pickup)
{
	//printf("------------target=%f\n",target[0]);
        if (node == NULL || kdnode_passed(tree, node)) {
			//printf("%d,,,%d\n",kdnode_passed(tree, node),node == NULL);
                return;
        }
